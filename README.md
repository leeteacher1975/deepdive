...
- 예외: Breakout Canvas는 "작성하기"/"함께 보기"가 한 파일(`breakout.html`) 안에 탭으로 구현되어 있어
  별도 `host/breakout.html`을 만들지 않았습니다. 대신 "함께 보기" 탭으로 전환할 때(URL에 직접
  `?tab=together`를 붙여 들어가는 경우 포함) 같은 비밀번호 게이트가 그 자리에서 뜹니다 — 그리고 "함께
  보기" 상태일 때는 플로우 탭의 나머지 탭들도 자동으로 `host/...` 주소로 바뀌어서, 진행자가 계속 진행자
  화면 사이만 이동하게 됩니다("작성하기" 상태에서는 다시 참가자 주소로 돌아옵니다).
...
├── breakout.html                    # ② Breakout Canvas — 작성하기(참가자)/함께 보기(진행자, 비밀번호 게이트) 탭
...
- 입력을 멈추면 약 1.2초 후 그 **칸 하나만** 자동 저장됩니다(별도 저장 버튼 없음, 칸마다 저장 상태 표시).
  ...
  추천합니다.
- "함께 보기" 탭은 진행자용 화면이라 처음 전환할 때 비밀번호를 확인합니다(2026-09-15 추가 — 다른
  활동들의 `/host/` 게이트와 동일한 방식). 통과하면 4개 요소를 색상이 다른 4개 컬럼으로 나란히 보여주고,
  2초 간격으로 자동 새로고침됩니다(탭이 보이지 않을 때는 폴링 일시중지). 이때 화면 상단 플로우 탭도
  자동으로 다른 활동의 진행자 화면(`host/...`)을 가리키도록 바뀝니다.
